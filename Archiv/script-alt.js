            ...
            try {
                ...
                const isMoodleSystem = (url) => {
                    const moodlePatterns = [
                        /\/mod\/((?!(data\/view\.php|page\/view\.php|forum\/view\.php)).)*$/, // Add forum/view.php to exceptions
                        /\/user\//,
                        /\/grade\//,
                        /\/group\//,
                        /\/message\//,
...
// ##### Kachel: Horizontal Accordion System #####
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ErrorHandler = {
        logError: (context, error) => {
            console.error(`IGEL Horizontal Accordion System - ${context}:`, error);
        }
    };

    // Horizontal Accordion Manager Object
        ...
        init: () => {
            ...
            try {
                ...
                if (horizontalAccordion) {
                    ...
                    console.log('No horizontal accordion containers found');
                    return;
                }
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    header.addEventListener('click', (event) => {
                        ...
                        return false; // Ensure we're not returning true accidentally
                    });
    ...
    // Initialize the horizontal accordion system
    try {
        // Check if there are horizontal accordion elements before initializing
        const hasHorizontalAccordion = document.querySelector(config.horizontalAccordionContainerSelector);

        if (hasHorizontalAccordion) {
